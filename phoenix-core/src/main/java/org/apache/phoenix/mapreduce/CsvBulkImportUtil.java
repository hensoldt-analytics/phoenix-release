...
public class CsvBulkImportUtil {
     ...
     */
    public static void initCsvImportJob(Configuration conf, Character fieldDelimiter, Character
            quoteChar, Character escapeChar, String arrayDelimiter, String binaryEncoding) {
    ...
    static void setChar(Configuration conf, String confKey, Character charValue) {
        if(charValue!=null) {
            conf.set(confKey, Bytes.toString(Base64.getEncoder().encode(Character.toString((char)charValue).getBytes())));
        }
    }
